...
# wilcoxon_tests(filter(comp_tbl_sample, sort_short_x == "CD45+"), cell_type, c("T cell", "B cell"), strain)
...
plot_comp_bar <- function(comp_tbl_rank, x, y, fill, nmax = 10000, facet = F, super_type = NULL, super_type_sub = NULL, highlight = F) {
  ...
  p <- ggplot(comp_tbl_rank, aes(!!x, !!y, fill = !!fill), color = NULL) +
  ...
  if (as_label(fill) %in% "cell_type") p <- p + scale_fill_manual(values = clrs[["cell_type_naila"]])
...
#               sample_id_rank, strain, strain, cell_type, "T cell", tiles_only = F, facet = sort_short_x)
# 
# plot_comp_box(rank_by(filter(comp_tbl_sample, sort_short_x == "CD45+"),
#                       cell_type, "T cell", cell_type) %>% mutate(site_helper = "Site"),
#               sample_id_lvl, site_helper, strain, cell_type, "T cell", tiles_only = T, facet = strain)
...
#                          cell_type, "T cell", cell_type), 
#                  sample_id_rank, sample_id,
#                  strain, strain, "Adnexa", 
#                  cell_type, "T cell")

default_comp_grid_list <- function(
  comp_tbl, rank_column, rank_value, fill_column, 
  n_bar = T, nrel_bar = T, mutsig_box = F, site_box = T, vec_plot = T,
  strain_tiles = F, mutsig_tiles = F, 
  ...
  # }
  if (site_box) {
    plist$pbox2 <- plot_comp_box(comp_tbl_rank, sample_id_rank, strain, 
                                 strain, !!rank_column, rank_value, 
                                 facet = !!facet, 
                                 post_filter_column = !!post_filter_column, 
                                 post_filter_value = post_filter_value) + 
      remove_guides
  ...
  if (strain_tiles) {
    plist$ptiles2 <- plot_comp_box(comp_tbl_rank, sample_id_lvl, label_strain, 
                                   strain, !!rank_column, rank_value, 
                                   facet = !!facet, 
                                   post_filter_column = NULL, 
                                   post_filter_value = NULL,
  ...
  if (vec_plot) {
    plist$pvec <- plot_comp_vector(comp_tbl_rank, sample_id_rank, sample_id,
                                   strain, strain, "Adnexa", 
                                   !!rank_column, rank_value) +
      remove_guides
  }
